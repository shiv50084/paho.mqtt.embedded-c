 ...
 *
 * This version of the API blocks on all method calls, until they are complete.  This means that
 * only one MQTT request can be in process at any one time.
...
public:
	 ...
	 *  @param network - pointer to an instance of the Network class - must be connected to the
	 * endpoint before calling MQTT connect
	...
	/** Set the default message handling callback - used for any message which does not match a
	 ...
	 *  @return success code -
	 */
	int publish(const char* topicName, void* payload, size_t payloadlen, enum QoS qos = QOS0,
	            bool retained = false);
	 ...
	 *  @return success code -
	 */
	int publish(const char* topicName, void* payload, size_t payloadlen, unsigned short& id,
	            enum QoS qos = QOS1, bool retained = false);

	/** MQTT Subscribe - send an MQTT subscribe packet and wait for the suback
	 *  @param topicFilter - a topic pattern which can include wildcards
	 *  @param qos - the MQTT QoS to subscribe at
	 *  @param mh - the callback function to be invoked when a message is received for this
	 * subscription
	 *  @return success code -
	 */
	...
	/** A call to this API must be made within the keepAlive interval to keep the MQTT connection
	 * alive yield can be called if no other MQTT operation is needed.  This will also allow
...
template <class Network, class Timer, int a, int MAX_MESSAGE_HANDLERS>
MQTT::Client<Network, Timer, a, MAX_MESSAGE_HANDLERS>::Client(Network& network,
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::readPacket(Timer& timer)
{
	...
	len += MQTTPacket_encode(readbuf + 1,
	                         rem_len); /* put the original remaining length into the buffer */
...
template <class Network, class Timer, int a, int MAX_MESSAGE_HANDLERS>
int MQTT::Client<Network, Timer, a, MAX_MESSAGE_HANDLERS>::deliverMessage(MQTTString& topicName,
                                                                          Message& message)
{
	int rc = FAILURE;
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::cycle(Timer& timer)
{
	...
	case PUBLISH:
	{
		...
		if (MQTTDeserialize_publish((unsigned char*)&msg.dup, &intQoS,
		                            (unsigned char*)&msg.retained, (unsigned short*)&msg.id,
		                            &topicName, (unsigned char**)&msg.payload,
		                            (int*)&msg.payloadlen, readbuf, MAX_MQTT_PACKET_SIZE)
		    != 1)
			goto exit;
	...
	case PUBREL:
		...
		if (MQTTDeserialize_ack(&type, &dup, &mypacketid, readbuf, MAX_MQTT_PACKET_SIZE) != 1)
			rc = FAILURE;
		            ...
		            MQTTSerialize_ack(sendbuf, MAX_MQTT_PACKET_SIZE,
	...
	case PUBCOMP: break;
#endif
	case PINGRESP: ping_outstanding = false; break;
	}

	if (keepalive() != SUCCESS)
		// check only keepalive FAILURE status so that previous FAILURE status can be considered as
		// FAULT
		rc = FAILURE;
                                                                   ...
                                                                   connackData& data)
{
	...
	if (waitfor(CONNACK, connect_timer) == CONNACK)
	{
		...
		if (MQTTDeserialize_connack((unsigned char*)&data.sessionPresent, (unsigned char*)&data.rc,
		                            readbuf, MAX_MQTT_PACKET_SIZE)
		    == 1)
			rc = data.rc;
		else
			rc = FAILURE;
...
template <class Network, class Timer, int MAX_MQTT_PACKET_SIZE, int MAX_MESSAGE_HANDLERS>
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, MAX_MESSAGE_HANDLERS>::subscribe(
  const char* topicFilter, enum QoS qos, messageHandler messageHandler, subackData& data)
{
	int rc = FAILURE;
	...
	len = MQTTSerialize_subscribe(sendbuf, MAX_MQTT_PACKET_SIZE, 0, packetid.getNext(), 1, &topic,
	                              (int*)&qos);
	if (len <= 0)
		goto exit;
	...
	if (waitfor(SUBACK, timer) == SUBACK) // wait for suback
	{
		...
		unsigned short mypacketid;
		data.grantedQoS = 0;
		if (MQTTDeserialize_suback(&mypacketid, 1, &count, &data.grantedQoS, readbuf,
...
template <class Network, class Timer, int MAX_MQTT_PACKET_SIZE, int MAX_MESSAGE_HANDLERS>
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, MAX_MESSAGE_HANDLERS>::unsubscribe(
  const char* topicFilter)
{
	int rc = FAILURE;
	Timer timer(command_timeout_ms);
	...
	if ((len = MQTTSerialize_unsubscribe(sendbuf, MAX_MQTT_PACKET_SIZE, 0, packetid.getNext(), 1,
	                                     &topic))
	    <= 0)
		goto exit;
...
template <class Network, class Timer, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::publish(int len, Timer& timer,
                                                                   enum QoS qos)
{
	int rc;
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::publish(const char* topicName,
                                                                   ...
                                                                   unsigned short& id, enum QoS qos,
                                                                   bool retained)
{
	int rc = FAILURE;
	Timer timer(command_timeout_ms);
	...
	len = MQTTSerialize_publish(sendbuf, MAX_MQTT_PACKET_SIZE, 0, qos, retained, id, topicString,
