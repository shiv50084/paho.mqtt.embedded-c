...
typedef struct
{
	int (*send)(unsigned char* address,
	            unsigned int bytes); ///< pointer to function to send 'bytes' bytes, returns the
	                                 ///< actual number of bytes sent
	int (*recv)(unsigned char* address,
	            unsigned int maxbytes); ///< pointer to function to receive upto 'maxbytes' bytes,
...
a built in timeout, if your system can provide this, do modify this function, otherwise use
getdatanb() instead
...
This function starts the process, you will call sendPacketBuffernb() until it reports success (or
error)
*/
...
/**
We assume whatever connection needs to be done, it is externally established by the specifics of the
hardware E.g.: A cell modem: you will call AT+whatever and put the modem in transparent mode, OR,
you will embed the AT+xSENDx / AT+xRECVx commands into the former sendPacketBuffer() and getdatanb()
functions
@param	thisio	pointer to a structure containing all necessary stuff to handle direct serial I/O
@returns	whatever indicator the system assigns to this link, if any. (a.k.a. : 'sock'), or
TRANSPORT_ERROR for error
*/
