...
int ThreadStart(Thread* thread, void (*fn)(void*), void* arg)
{
	...
	UBaseType_t uxTaskPriority =
	  uxTaskPriorityGet(NULL); /* set the priority as the same as the calling task*/
...
void TimerCountdownMS(Timer* timer, unsigned int timeout_ms)
{
	...
	vTaskSetTimeOutState(
	  &timer->xTimeOut); /* Record the time at which this function was entered. */
}
...
{
	xTaskCheckForTimeOut(&timer->xTimeOut,
...
int FreeRTOS_read(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		...
		FreeRTOS_setsockopt(
		  n->my_socket, 0, FREERTOS_SO_RCVTIMEO, &xTicksToWait, sizeof(xTicksToWait));
