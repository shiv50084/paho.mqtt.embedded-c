	...
	case PUBREL:
	{
		...
		if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf, c->readbuf_size) != 1)
			rc = FAILURE;
		            ...
		            c->buf, c->buf_size, (packet_type == PUBREC) ? PUBREL : PUBCOMP, 0, mypacketid))
		         <= 0)
			rc = FAILURE;
                             ...
                             messageHandler messageHandler, MQTTSubackData* data)
{
	...
	len =
	  MQTTSerialize_subscribe(c->buf, c->buf_size, 0, getNextPacketId(c), 1, &topic, (int*)&qos);
	if (len <= 0)
		goto exit;
	...
	if (waitfor(c, SUBACK, &timer) == SUBACK) // wait for suback
	{
		...
		if (MQTTDeserialize_suback(&mypacketid, 1, &count, (int*)&data->grantedQoS, c->readbuf,
		                           c->readbuf_size)
		    == 1)
		{
...
int MQTTUnsubscribe(MQTTClient* c, const char* topicFilter)
{
	...
	if ((len = MQTTSerialize_unsubscribe(c->buf, c->buf_size, 0, getNextPacketId(c), 1, &topic))
	    <= 0)
		goto exit;
	if ((rc = sendPacket(c, len, &timer)) != SUCCESS) // send the subscribe packet
		goto exit;                                    // there was a problem
...
int MQTTPublish(MQTTClient* c, const char* topicName, MQTTMessage* message)
{
	...
	len =
	  MQTTSerialize_publish(c->buf, c->buf_size, 0, message->qos, message->retained, message->id,
